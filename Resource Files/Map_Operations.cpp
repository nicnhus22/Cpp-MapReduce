...
std::unordered_map <std::string, int> getFrequency(const std::vector<std::string> & FILE_CONTENT, std::unordered_map<std::string, int> & MASTER,unordered_map<string, int> & DOC_APPEAR_SENTENCES){
    ...
    for(vector<string>::size_type j = 0; j != FILE_CONTENT.size(); ++j){
        ...
        if ( MAP_MASTER_IT == MASTER.end() ){ // If not in the MASTER add it without incrementing
            MASTER[TEMP] = 1;
            DOC_APPEAR_SENTENCES[TEMP] = 0;
        }else{ // If it is in the MAP then increment and add it
            MASTER[TEMP] = MASTER[TEMP]+1;
        }

        if ( MAP_IT == MAP.end() ){ // If not in the MAP add it without incrementing
            DOC_APPEAR_SENTENCES[TEMP] += 1;
            MAP[TEMP] = 1;
        }else{ // If it is in the MAP then increment and add it
            MAP[TEMP] = MAP[TEMP]+1;
        }
 ...
 *	an unordered_map<string, int> where the key is the string and the value its frequency
 */
std::unordered_map <std::string, int> getQueryFrequency(const std::vector<std::string> & FILE_CONTENT, std::unordered_map<std::string, int> & MASTER,unordered_map<string, int> & DOC_APPEAR_SENTENCES){
    unordered_map <string, int> MAP;
...
/*
 * Checks if a word is in a doc (if frequency of word in doc is bigger than 0)
 *
 */
bool isInDocument(const string & s, unordered_map<string,int> map){
    if(map[s] > 0)
        return true;
    return false;
}
